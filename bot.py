...
def main():
	...
	def text(m, res=False):
		...
		if m.text == '⌚️ Бэкапы 🕓':
			...
			bot.send_message(m.chat.id, "Этой функции пока нет.")
		elif m.text == '💧 Погода ⛅':
			try:
				city = "Lysva"
				r = requests.get(f'https://api.openweathermap.org/data/2.5/weather?q={city}&appid={w_token}&units=metric')
				data = r.json()
				city = data["name"]
			...
			try:
				...
				div = text.findAll("div", {'class' : 'cv-countdown__item-value _accent'})
				div_h = text.find("div", {"class" : "cv-countdown__item-value _accent-green"})
				timee()
				bot.send_message(m.chat.id, "Данные на " + cur_date + " " + cur_time + "\n😓 Госпитализировно: " + div[0].text + "\n🤒 Выявлено: " + div[1].text + "\n😃 Выздоравлено: " + div_h.text)
			except Exception:
				timee()
			...
			gachi_list = ['/home/pixart/Kaneka/video/v1.mp4', '/home/pixart/Kaneka/video/v2.mp4', '/home/pixart/Kaneka/video/v3.mp4', '/home/pixart/Kaneka/video/v4.mp4', '/home/pixart/Kaneka/video/v5.mp4', '/home/pixart/Kaneka/video/v6.mp4']
